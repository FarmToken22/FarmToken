// bonus.js - Enhanced 30-Day Streak with Smart Ad Management (Error-Free)
...
export function renderBonusSection() {
    ...
    const cardsHTML = Array.from({ length: TOTAL_DAYS }, (_, i) => {
        ...
        return '<div class="day-card group relative overflow-hidden rounded-2xl border-2 border-gray-200 bg-gradient-to-br from-white to-gray-50 shadow-sm hover:shadow-xl hover:scale-105 transition-all duration-300 cursor-pointer" data-day="' + day + '">' +
            (isSpecialDay ? '<div class="absolute -top-1 -right-1 w-12 h-12 bg-gradient-to-br from-yellow-400 to-orange-500 rounded-bl-3xl flex items-center justify-center">' +
            ...
            '<div class="relative p-4 text-center">' +
                ...
                '<div class="relative inline-block">' +
                    ...
                    '<p class="relative text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-yellow-500 to-orange-600">+' + reward + '</p>' +
                '</div>' +
            ...
            '<div class="next-indicator absolute inset-0 pointer-events-none opacity-0 transition-opacity duration-500">' +
                ...
                '<div class="absolute -top-2 left-1/2 -translate-x-1/2 bg-yellow-400 text-white text-xs font-bold px-3 py-1 rounded-full shadow-lg">Next!</div>' +
            '</div>' +
        '</div>';
    }).join('');
    ...
    container.innerHTML = 
        '<div class="p-4 sm:p-6 max-w-5xl mx-auto w-full">' +
            ...
            '<div class="text-center mb-8 relative">' +
                ...
                '</div>' +
                '<div class="relative">' +
                    '<h2 class="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-yellow-500 via-orange-500 to-red-500 mb-2 animate-gradient">30-Day Streak Challenge</h2>' +
                ...
                '<div class="relative grid grid-cols-1 md:grid-cols-3 gap-6">' +
                    ...
                    '<div class="text-center">' +
                        '<div class="inline-flex items-center justify-center w-16 h-16 bg-gradient-to-br from-blue-400 to-indigo-500 rounded-full mb-3 shadow-lg">' +
                            '<svg class="w-8 h-8 text-white" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd"/></svg>' +
                        '</div>' +
            ...
            '<!-- 30 Cards Grid -->' +
            '<div class="grid grid-cols-3 gap-3 mb-6">' + cardsHTML + '</div>' +

            '<!-- Ad Space 1 -->' +
            '<div id="adSpace1" class="ad-container mb-6 min-h-[50px] flex items-center justify-center bg-gray-50 rounded-xl overflow-hidden transition-all duration-300" style="display: none;">' +
            ...
            '<button id="claimBonusBtn" class="w-full relative overflow-hidden bg-gradient-to-r from-yellow-500 via-orange-500 to-red-500 text-white py-4 rounded-2xl shadow-2xl text-lg font-black hover:shadow-yellow-500/50 hover:scale-105 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 group">' +
                ...
                '<span class="relative flex items-center justify-center gap-3">' +
                    '<svg id="claimBtnIcon" class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20"><path d="M5 4a2 2 0 012-2h6a2 2 0 012 2v14l-5-2.5L5 18V4z"/></svg>' +
                    '<span id="claimBtnText">Claim Your Daily Reward</span>' +
                    '<span id="claimLoader" class="hidden"><svg class="animate-spin h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">' +
            ...
            '<!-- Ad Space 2 -->' +
            '<div id="adSpace2" class="ad-container mt-6 mb-4 min-h-[50px] flex items-center justify-center bg-gray-50 rounded-xl overflow-hidden transition-all duration-300" style="display: none;">' +
        ...
        '<style>' +
            ...
            '.day-card:hover .progress-bar { box-shadow: 0 0 20px rgba(234, 179, 8, 0.5); }' +
            '.ad-container { min-height: 50px; transition: all 0.3s ease; }' +
        '</style>';

    console.log('Enhanced 30-Day Streak rendered with smart ad system');
    loadSingleAd();
}
...
function loadSingleAd() {
    ...
    script.onload = () => {
        adLoaded = true;
        console.log('Ad loaded in bonus adSpace' + (randomIndex + 1));
    };
    script.onerror = () => {
        selected.space.style.display = 'none';
        console.warn('Ad failed to load in bonus adSpace' + (randomIndex + 1));
    };

    window.atOptions = { 'key': AD_KEY, 'format': 'iframe', 'height': 50, 'width': 320, 'params': {} };
    script.src = AD_URL;
...
export function updateBonusTimer(userData, getServerTime) {
    ...
    const update = () => {
        ...
        nextBonusAmountEl.innerHTML = state.nextBonusAmount + ' FZ Gift';
        streakDisplayEl.textContent = state.currentStreak + ' day' + (state.currentStreak !== 1 ? 's' : '') + ' Fire';
...
export async function claimBonus(currentUser, userData, getServerTime, showNotification) {
    ...
    try {
        ...
        showStatus(statusEl, 'Success! +' + bonusAmount + ' FZ Added! Success!', false);
...
function createConfetti() {
    ...
    for (let i = 0; i < confettiCount; i++) {
        ...
        confetti.style.cssText = 'position:fixed;width:10px;height:10px;background:' + colors[Math.floor(Math.random() * colors.length)] + ';top:50%;left:50%;opacity:1;border-radius:50%;pointer-events:none;z-index:9999;';
...
export function initBonusSection(userData, getServerTime) {
    ...
    const claimBtn = document.getElementById('claimBonusBtn');
    if (claimBtn) {
        claimBtn.onclick = () => claimBonus(auth.currentUser, userData, getServerTime, window.showNotification);
    }

    console.log('Enhanced 30-Day Streak initialized with smart ad system');
}
